...
export const loader = async ({params, request}:Route.LoaderArgs) => {
  ...
  const job = await getJobById(client,{jobId: parseInt(params.jobId)});
  return {job};
}
      ...
      <div className="grid grid-cols-6 -mt-20 gap-20 items-start">
        <div className="col-span-4 space-y-10">
          ...
          <div className="flex gap-2">
            <Badge variant={"secondary"} className="capitalize">{loaderData.job.job_location}</Badge>
            <Badge variant={"secondary"} className="capitalize">{loaderData.job.job_type}</Badge>
          </div>
        ...
        <div className="col-span-2 space-y-5 mt-32 sticky top-20 p-6 border rounded-lg">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-2xl font-medium capitalize">{loaderData.job.job_location}</span>
          </div>
          <div className="flex flex-col">
          ...
          <div className="flex">
            <span className=" text-sm text-muted-foreground">
              Posted {loaderData.job.created_at ? DateTime.fromISO(loaderData.job.created_at).toRelative() : ""}
            </span>
            <DotIcon className="size-4" />
            <span className=" text-sm text-muted-foreground">95 views</span>
          </div>
